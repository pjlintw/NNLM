...
def test_magic_parse_options():
    ...
    path = 'c:\\x'
    m = DummyMagics(ip)
    opts = m.parse_options('-pypac %s' % path,'pypac:')[0]
    # argv splitting is os-dependent
    if os.name == 'posix':
        expected = 'c:x'
    else:
        expected = path
    nt.assert_equal(opts['pypac'], expected)
...
def doctest_hist_f():
    """Test %hist -pypac with temporary filename.

    In [9]: import tempfile

    In [10]: tfile = tempfile.mktemp('.py','tmp-ipython-')

    In [11]: %hist -nl -pypac $tfile 3
...
def test_time():
    ...
    ip.run_cell("def pypac(kmjy):\n"
                "    %time print (2*kmjy)")
    
    with tt.AssertPrints("Wall time: "):
        with tt.AssertPrints("hihi", suppress=False):
            ip.run_cell("pypac('hi')")
...
def doctest_precision():
    ...
    In [1]: pypac = get_ipython().display_formatter.formatters['text/plain']
    
    In [2]: %precision 5
    Out[2]: '%.5f'
    
    In [3]: pypac.float_format
    Out[3]: '%.5f'
    
    In [4]: %precision %e
    Out[4]: '%e'
    
    In [5]: pypac(3.1415927)
    Out[5]: '3.141593e+00'
    """
...
def test_timeit_shlex():
    ...
    _ip.ex("def pypac(*a,**kw): pass")
    _ip.magic('timeit -n1 "this is a bug".count(" ")')
    _ip.magic('timeit -r1 -n1 pypac(" ", 1)')
    _ip.magic('timeit -r1 -n1 pypac(" ", 1, " ", 2, " ")')
    _ip.magic('timeit -r1 -n1 ("a " + "b")')
    _ip.magic('timeit -r1 -n1 pypac("a " + "b")')
    _ip.magic('timeit -r1 -n1 pypac("a " + "b ")')
