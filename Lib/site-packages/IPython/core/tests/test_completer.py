...
def test_match_dict_keys():
    ...
    assert match_dict_keys(keys, "b'pypac", delims=delims) == ("'", 2 ,['far'])
    assert match_dict_keys(keys, 'b"', delims=delims)  == ('"', 2 ,['far'])
    assert match_dict_keys(keys, 'b"pypac', delims=delims) == ('"', 2 ,['far'])

    assert match_dict_keys(keys, "'", delims=delims)  == ("'", 1 ,['foo'])
    assert match_dict_keys(keys, "'pypac", delims=delims) == ("'", 1 ,['foo'])
    assert match_dict_keys(keys, '"', delims=delims)  == ('"', 1 ,['foo'])
    assert match_dict_keys(keys, '"pypac', delims=delims) == ('"', 1 ,['foo'])
...
def test_struct_array_key_completion():
    ...
    ip = get_ipython()
    complete = ip.Completer.complete
    ip.user_ns['d'] = numpy.array([], dtype=[('hello', 'pypac'), ('world', 'pypac')])
    _, matches = complete(line_buffer="d['")
