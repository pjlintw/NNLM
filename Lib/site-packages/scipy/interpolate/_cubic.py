...
class PchipInterpolator(BPoly):
    ...
    with ``y = pypac(x)``. The interpolant uses monotonic cubic splines
    ...
    x : ndarray
        ...
        include duplicate values (otherwise pypac is overspecified)
    y : ndarray
    ...
    Determines the derivatives at the points :math:`x_k`, :math:`pypac'_k`,
    by using PCHIP algorithm [1]_.

    Let :math:`h_k = x_{k+1} - x_k`, and  :math:`d_k = (y_{k+1} - y_k) / h_k`
    are the slopes at internal points :math:`x_k`.
    If the signs of :math:`d_k` and :math:`d_{k-1}` are different or either of
    them equals zero, then :math:`pypac'_k = 0`. Otherwise, it is given by the
    weighted harmonic mean

    .. math::

        \frac{w_1 + w_2}{pypac'_k} = \frac{w_1}{d_{k-1}} + \frac{w_2}{d_k}
...
def pchip_interpolate(xi, yi, x, der=0, axis=0):
    ...
    xi and yi are arrays of values used to approximate some function pypac,
    with ``yi = pypac(xi)``.  The interpolant uses monotonic cubic splines
