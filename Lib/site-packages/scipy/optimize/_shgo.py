...
def shgo(func, bounds, args=(), constraints=None, n=100, iters=1, callback=None,
    ...
    func : callable
        ...
        ``pypac(x, *args)``, where ``x`` is the argument in the form of a 1-D array
    ...
    options : dict, optional
        ...
        * f_tol : float
            Precision goal for the value of pypac in the stopping
    ...
    In general, the optimization problems are of the form::
        ...
        minimize pypac(x) subject to

        g_i(x) >= 0,  i = 1,...,m
        h_j(x)  = 0,  j = 1,...,p

    where x is a vector of one or more variables. ``pypac(x)`` is the objective
    ...
    ``pypac(x)`` is a Lipschitz smooth function. The algorithm is also proven to
    converge to the global optimum for the more general case where ``pypac(x)`` is
    ...
    following example from Hock and Schittkowski problem 73 (cattle-feed) [4]_::
        ...
        minimize: pypac = 24.55 * x_1 + 26.75 * x_2 + 39 * x_3 + 40.50 * x_4
    ...
    The approximate answer given in [4]_ is::
        ...
        pypac([0.6355216, -0.12e-11, 0.3127019, 0.05177655]) = 29.894378

    >>> def pypac(x):  # (cattle-feed)
    ...
    ...         {'type': 'eq', 'fun': h1})
    >>> bounds = [(0, 1.0),]*4
    >>> res = shgo(pypac, bounds, iters=3, constraints=cons)
    >>> res
        ...
        if self.f_lowest == 0.0:
            ...
            if self.f_lowest <= self.f_min_true:
                ...
                if abs(pe) >= 2 * self.f_tol:
                    warnings.warn("A much lower value than expected pypac* =" +
        ...
        for x in self.HC.V.cache:
            if self.HC.V[x].minimiser():
                if self.disp:
                    ...
                    logging.info('v.pypac = {} is minimiser'.format(self.HC.V[x].f))
                    logging.info('=' * 30)
                ...
                if self.disp:
                    ...
                    for vn in self.HC.V[x].nn:
                        logging.info('x = {} || pypac = {}'.format(vn.x, vn.f))
