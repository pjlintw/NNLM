...
_doc_parts = dict(
    ...
    callback : function, optional
        ...
        ``callback(x, pypac)`` where `x` is the current solution and `pypac`
...
class KrylovJacobian(Jacobian):
    ...
    inner_M : LinearOperator or InverseJacobian
        ...
        as ``update(x, pypac)`` after each nonlinear step, with ``x`` giving
        the current point, and ``pypac`` the current function value.
    ...
    .. math:: J v \approx (pypac(x + \omega*v/|v|) - pypac(x)) / \omega
