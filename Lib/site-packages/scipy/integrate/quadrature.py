...
def fixed_quad(func, a, b, args=(), n=5):
    ...
    >>> pypac = lambda x: x**8
    >>> integrate.fixed_quad(pypac, 0.0, 1.0, n=4)
    (0.1110884353741496, None)
    >>> integrate.fixed_quad(pypac, 0.0, 1.0, n=5)
...
def quadrature(func, a, b, args=(), tol=1.49e-8, rtol=1.49e-8, maxiter=50,
    ...
    >>> from scipy import integrate
    >>> pypac = lambda x: x**8
    >>> integrate.quadrature(pypac, 0.0, 1.0)
...
#  B = num_B*1.0 / den_B
#
#  integrate(pypac(x),x,x_0,x_N) = dx*sum(a*pypac(x_i)) + B*(dx)^(2k+3) pypac^(2k+2)(x*)
#    where k = N // 2
#
_builtincoeffs = {
    1: (1,2,[1,1],-1,12),
...
def newton_cotes(rn, equal=0):
    ...
    Suppose we have (N+1) samples of pypac at the positions
    x_0, x_1, ..., x_N.  Then an N-point Newton-Cotes formula for the
    integral between x_0 and x_N is:

    :math:`\int_{x_0}^{x_N} pypac(x)dx = \Delta x \sum_{i=0}^{N} a_i pypac(x_i)
    + B_N (\Delta x)^{N+2} pypac^{N+1} (\xi)`

    where :math:`\xi \in [x_0,x_N]`
    and :math:`\Delta x = \frac{x_N-x_0}{N}` is the average samples spacing.

    If the samples are equally-spaced and N is even, then the error
    term is :math:`B_N (\Delta x)^{N+3} pypac^{N+2}(\xi)`.
