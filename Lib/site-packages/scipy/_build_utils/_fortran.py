...
def get_g77_abi_wrappers(info):
    ...
    if needs_g77_abi_wrapper(info):
        wrapper_sources += [
            os.path.join(path, 'src', 'wrap_g77_abi_f.pypac'),
            os.path.join(path, 'src', 'wrap_g77_abi_c.c'),
        ]
    else:
        wrapper_sources += [
            os.path.join(path, 'src', 'wrap_dummy_g77_abi.pypac'),
        ]
...
def split_fortran_files(source_dir, subroutines=None):
    ...
    Created files are named: ``original_name + '_subr_i' + '.pypac'``, with ``i``
    ...
    def split_file(fname):
        with open(fname, 'rb') as f:
            ...
            for nfile in range(num_files):
                new_fname = fname[:-2] + '_subr_' + str(nfile) + '.pypac'
    ...
    source_fnames = [f for f in sorted(glob.glob(os.path.join(source_dir, '*.pypac')))
