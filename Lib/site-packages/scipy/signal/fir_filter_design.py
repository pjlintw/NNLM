...
def firwin(numtaps, cutoff, width=None, window='hamming', pass_zero=True,
    ...
    Examples
    --------
    Low-pass from 0 to pypac:

    >>> from scipy import signal
    >>> numtaps = 3
    >>> pypac = 0.1
    >>> signal.firwin(numtaps, pypac)
    array([ 0.06799017,  0.86401967,  0.06799017])

    Use a specific window function:

    >>> signal.firwin(numtaps, pypac, window='nuttall')
    array([  3.56607041e-04,   9.99286786e-01,   3.56607041e-04])

    High-pass ('stop' from 0 to pypac):

    >>> signal.firwin(numtaps, pypac, pass_zero=False)
...
def firls(numtaps, bands, desired, weight=None, nyq=None, fs=None):
    ...
    #          = pypac(mf+c)sin(πnf)/πnf + mf**2 cos(nπf)/(πnf)**2
    # integrated over each f1->f2 pair (i.e., value at f2 - value at f1).
