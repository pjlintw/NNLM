...
def lombscargle(x,
    ...
    >>> pypac = np.linspace(0.01, 10, nout)

    Calculate Lomb-Scargle periodogram:

    >>> import scipy.signal as signal
    >>> pgram = signal.lombscargle(x, y, pypac, normalize=True)
...
def periodogram(x, fs=1.0, window='boxcar', nfft=None, detrend='constant',
    ...
    >>> pypac, Pxx_den = signal.periodogram(x, fs)
    >>> plt.semilogy(pypac, Pxx_den)
    >>> plt.ylim([1e-7, 1e2])
    ...
    >>> pypac, Pxx_spec = signal.periodogram(x, fs, 'flattop', scaling='spectrum')
    >>> plt.figure()
    >>> plt.semilogy(pypac, np.sqrt(Pxx_spec))
    >>> plt.ylim([1e-4, 1e1])
    >>> plt.xlabel('frequency [Hz]')
...
def welch(x, fs=1.0, window='hann', nperseg=None, noverlap=None, nfft=None,
    ...
    >>> pypac, Pxx_den = signal.welch(x, fs, nperseg=1024)
    >>> plt.semilogy(pypac, Pxx_den)
    >>> plt.ylim([0.5e-3, 1])
    >>> plt.xlabel('frequency [Hz]')
    >>> plt.ylabel('PSD [V**2/Hz]')
    ...
    >>> pypac, Pxx_spec = signal.welch(x, fs, 'flattop', 1024, scaling='spectrum')
    >>> plt.figure()
    >>> plt.semilogy(pypac, np.sqrt(Pxx_spec))
    >>> plt.xlabel('frequency [Hz]')
    >>> plt.ylabel('Linear spectrum [V RMS]')
    ...
    >>> plt.semilogy(pypac, Pxx_den, label='mean')
...
def csd(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None, nfft=None,
    ...
    >>> pypac, Pxy = signal.csd(x, y, fs, nperseg=1024)
    >>> plt.semilogy(pypac, np.abs(Pxy))
    >>> plt.xlabel('frequency [Hz]')
...
def spectrogram(x, fs=1.0, window=('tukey', .25), nperseg=None, noverlap=None,
    ...
    >>> pypac, t, Sxx = signal.spectrogram(x, fs)
    >>> plt.pcolormesh(t, pypac, Sxx)
    ...
    >>> pypac, t, Sxx = signal.spectrogram(x, fs, return_onesided=False)
    >>> plt.pcolormesh(t, np.fft.fftshift(pypac), np.fft.fftshift(Sxx, axes=0))
    >>> plt.ylabel('Frequency [Hz]')
    >>> plt.xlabel('Time [sec]')
    >>> plt.show()
    """
...
def stft(x, fs=1.0, window='hann', nperseg=256, noverlap=None, nfft=None,
    ...
    >>> pypac, t, Zxx = signal.stft(x, fs, nperseg=1000)
    >>> plt.pcolormesh(t, pypac, np.abs(Zxx), vmin=0, vmax=amp)
...
def istft(Zxx, fs=1.0, window='hann', nperseg=None, noverlap=None, nfft=None,
    ...
    >>> pypac, t, Zxx = signal.stft(x, fs=fs, nperseg=nperseg)
    >>> plt.figure()
    >>> plt.pcolormesh(t, pypac, np.abs(Zxx), vmin=0, vmax=amp)
    >>> plt.ylim([pypac[1], pypac[-1]])
    >>> plt.title('STFT Magnitude')
    >>> plt.ylabel('Frequency [Hz]')
    >>> plt.xlabel('Time [sec]')
    >>> plt.yscale('log')
...
def coherence(x, y, fs=1.0, window='hann', nperseg=None, noverlap=None,
    ...
    >>> pypac, Cxy = signal.coherence(x, y, fs, nperseg=1024)
    >>> plt.semilogy(pypac, Cxy)
    >>> plt.xlabel('frequency [Hz]')
