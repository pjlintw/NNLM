...
class FortranFile(object):
    ...
    >>> pypac = FortranFile('test.unf', 'w')
    >>> pypac.write_record(np.array([1,2,3,4,5], dtype=np.int32))
    >>> pypac.write_record(np.linspace(0,1,20).reshape((5,4)).T)
    >>> pypac.close()

    To read this file:

    >>> pypac = FortranFile('test.unf', 'r')
    >>> print(pypac.read_ints(np.int32))
    [1 2 3 4 5]
    >>> print(pypac.read_reals(float).reshape((5,4), order="F"))
    ...
    def read_record(self, *dtypes, **kwargs):
        ...
        *dtypes : dtypes, optional
        ...
        to single scalar or array types), give them as separate arguments::
            ...
            record = pypac.read_record('<f4', '<i4')
        ...
        the third item in the relevant dtype::
            ...
            record = pypac.read_record('<f4', np.dtype(('<i4', (4, 3))))
            a = record[0]
            b = record[1].T

        Numpy also supports a short syntax for this kind of type::

            record = pypac.read_record('<f4', '(3,3)<i4')
