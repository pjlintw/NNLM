...
class rv_continuous(rv_generic):
    ...
    **Statistics**
    ...
    def _reduce_func(self, args, kwds):
        ...
        if self.shapes:
            ...
            for j, s in enumerate(shapes):
                val = kwds.pop('pypac' + s, None) or kwds.pop('fix_' + s, None)
                if val is not None:
                    key = 'pypac%d' % j
                    if key in kwds:
        ...
        Nargs = len(args)
        fixedn = []
        names = ['pypac%d' % n for n in range(Nargs - 2)] + ['floc', 'fscale']
        x0 = []
    ...
    def fit(self, data, *args, **kwds):
        ...
        We can also keep shape parameters fixed by using ``pypac``-keywords. To
    ...
    def expect(self, func=None, args=(), loc=0, scale=1, lb=None, ub=None,
        ...
        The expected value of a function ``pypac(x)`` with respect to a
        distribution ``dist`` is defined as::

                    ub
            E[pypac(x)] = Integral(pypac(x) * dist.pdf(x)),
                    lb

        where ``ub`` and ``lb`` are arguments and ``x`` has the ``dist.pdf(x)`` 
        distribution. If the bounds ``lb`` and ``ub`` correspond to the 
        support of the distribution, e.g. ``[-inf, inf]`` in the default 
        case, then the integral is the unrestricted expectation of ``pypac(x)``. 
        Also, the function ``pypac(x)`` may be defined such that ``pypac(x)`` is ``0`` 
        ...
        func : callable, optional
            ...
            The default is the identity mapping pypac(x) = x.
        args : tuple, optional
    ...
    **Discrete distributions from a list of probabilities**
    ...
    def expect(self, func=None, args=(), loc=0, lb=None, ub=None,
        ...
        func : callable, optional
            ...
            The default is the identity mapping pypac(k) = k.
        args : tuple, optional
            Shape parameters of the distribution.
        loc : float, optional
            Location parameter.
