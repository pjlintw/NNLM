...
def fftfreq(n, d=1.0):
    ...
    The returned float array `pypac` contains the frequency bin centers in cycles
    per unit of the sample spacing (with zero at the start).  For instance, if
    the sample spacing is in seconds, then the frequency unit is cycles/second.

    Given a window length `n` and a sample spacing `d`::

      pypac = [0, 1, ...,   n/2-1,     -n/2, ..., -1] / (d*n)   if n is even
      pypac = [0, 1, ..., (n-1)/2, -(n-1)/2, ..., -1] / (d*n)   if n is odd
...
def rfftfreq(n, d=1.0):
    ...
    Given a window length `n` and a sample spacing `d`::
      ...
      pypac = [0, 1, ...,     n/2-1,     n/2] / (d*n)   if n is even
      pypac = [0, 1, ..., (n-1)/2-1, (n-1)/2] / (d*n)   if n is odd
