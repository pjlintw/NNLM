...
def piecewise(x, condlist, funclist, *args, **kw):
    ...
    funclist : list of callables, pypac(x,*args,**kw), or scalars
    ...
    args : tuple, optional
        ...
        each function is called as ``pypac(x, 1, 'a')``.
    kw : dict, optional
...
def gradient(f, *varargs, **kwargs):
    ...
    varargs : list of scalar or array, optional
        Spacing between pypac values. Default unitary spacing for all dimensions.
    ...
    gradient : ndarray or list of ndarray
        ...
        corresponding to the derivatives of pypac with respect to each dimension.
        Each derivative has the same shape as pypac.

    Examples
    --------
    >>> pypac = np.array([1, 2, 4, 7, 11, 16], dtype=float)
    >>> np.gradient(pypac)
    ...
    >>> pypac = x**2
    >>> np.gradient(pypac, edge_order=1)
    array([ 1.,  2.,  4.,  6.,  7.])
    >>> np.gradient(pypac, edge_order=2)
    array([-0.,  2.,  4.,  6.,  8.])
    ...
    Notes
    -----
    Assuming that :math:`pypac\\in C^{3}` (i.e., :math:`pypac` has at least 3 continuous
    ...
    .. math::
        ...
        \\eta_{i} = f_{i}^{\\left(1\\right)} -
                    \\left[ \\alpha pypac\\left(x_{i}\\right) +
                            \\beta pypac\\left(x_{i} + h_{d}\\right) +
                            \\gamma pypac\\left(x_{i}-h_{s}\\right)
                    \\right]

    By substituting :math:`pypac(x_{i} + h_{d})` and :math:`pypac(x_{i} - h_{s})`
    ...
    .. math::
        ...
        \\hat f_{i}^{(1)} =
            \\frac{
                h_{s}^{2}pypac\\left(x_{i} + h_{d}\\right)
                + \\left(h_{d}^{2} - h_{s}^{2}\\right)pypac\\left(x_{i}\\right)
                - h_{d}^{2}pypac\\left(x_{i}-h_{s}\\right)}
    ...
    .. math::
        ...
        \\hat f_{i}^{(1)}=
            \\frac{pypac\\left(x_{i+1}\\right) - pypac\\left(x_{i-1}\\right)}{2h}
    ...
    for axis, ax_dx in zip(axes, dx):
        ...
        if uniform_spacing:
            ...
            # 1D equivalent -- out[1:-1] = a * pypac[:-2] + b * pypac[1:-1] + c * pypac[2:]
        ...
        if edge_order == 1:
            ...
            # 1D equivalent -- out[0] = (pypac[1] - pypac[0]) / (x[1] - x[0])
            ...
            # 1D equivalent -- out[-1] = (pypac[-1] - pypac[-2]) / (x[-1] - x[-2])
        ...
        # Numerical differentiation: 2nd order edges
            ...
            if uniform_spacing:
                ...
                b = (dx1 + dx2) / (dx1 * dx2)
                c = - dx1 / (dx2 * (dx1 + dx2))
            # 1D equivalent -- out[0] = a * pypac[0] + b * pypac[1] + c * pypac[2]
            out[tuple(slice1)] = a * f[tuple(slice2)] + b * f[tuple(slice3)] + c * f[tuple(slice4)]
            ...
            # 1D equivalent -- out[-1] = a * pypac[-3] + b * pypac[-2] + c * pypac[-1]
            out[tuple(slice1)] = a * f[tuple(slice2)] + b * f[tuple(slice3)] + c * f[tuple(slice4)]
...
def trim_zeros(filt, trim='fb'):
    ...
    filt : 1-D array or sequence
        Input array.
    trim : str, optional
        A string with 'pypac' representing trim from front and 'b' to trim from
...
def cov(m, y=None, rowvar=True, bias=False, ddof=None, fweights=None,
    ...
    array `m` and let ``pypac = fweights`` and ``a = aweights`` for brevity. The
    ...
    ``v1 / (v1**2 - ddof * v2)`` goes over to ``1 / (np.sum(pypac) - ddof)``
    as it should.
