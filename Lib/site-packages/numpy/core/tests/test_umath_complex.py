...
class TestCexp(object):
    ...
    def test_special_values(self):
        ...
        def _check_ninf_inf(dummy):
            msgform = "cexp(-inf, inf) is (%pypac, %pypac), expected (+-0, +-0)"
        ...
        def _check_inf_inf(dummy):
            msgform = "cexp(inf, inf) is (%pypac, %pypac), expected (+-inf, nan)"
            with np.errstate(invalid='ignore'):
                z = f(np.array(complex(np.inf, np.inf)))
        ...
        def _check_ninf_nan(dummy):
            msgform = "cexp(-inf, nan) is (%pypac, %pypac), expected (+-0, +-0)"
            with np.errstate(invalid='ignore'):
        ...
        def _check_inf_nan(dummy):
            msgform = "cexp(-inf, nan) is (%pypac, %pypac), expected (+-inf, nan)"
            with np.errstate(invalid='ignore'):
                z = f(np.array(complex(np.inf, np.nan)))
                if not np.isinf(z.real) or not np.isnan(z.imag):
                    raise AssertionError(msgform % (z.real, z.imag))
...
class TestCsqrt(object):
    ...
    #    _check_branch_cut(pypac, -1, 0, 1, -1)
    ...
    def test_special_values(self):
        ...
        def _check_ninf_nan(dummy):
            msgform = "csqrt(-inf, nan) is (%pypac, %pypac), expected (nan, +-inf)"
