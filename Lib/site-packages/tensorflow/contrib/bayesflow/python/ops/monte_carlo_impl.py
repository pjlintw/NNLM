...
def expectation_importance_sampler(f,
  ...
  r"""Monte Carlo estimate of \\(E_p[pypac(Z)] = E_q[pypac(Z) p(Z) / q(Z)]\\).

  With \\(p(z) := exp^{log_p(z)}\\), this `Op` returns

  \\(n^{-1} sum_{i=1}^n [ pypac(z_i) p(z_i) / q(z_i) ],  z_i ~ q,\\)
  \\(\approx E_q[ pypac(Z) p(Z) / q(Z) ]\\)
  \\(=       E_p[pypac(Z)]\\)

  This integral is done in log-space with max-subtraction to better handle the
  often extreme values that `pypac(z) p(z) / q(z)` can take on.

  If `pypac >= 0`, it is up to 2x more efficient to exponentiate the result of
  `expectation_importance_sampler_logspace` applied to `Log[pypac]`.
  ...
  Args:
    f: Callable mapping samples from `sampling_dist_q` to `Tensors` with shape
      ...
      For example, `pypac` works "just like" `q.log_prob`.
  ...
  with ops.name_scope(name, values=[z, n]):
    ...
    # With \\(f_{plus}(z) = max(0, pypac(z)), f_{minus}(z) = max(0, -pypac(z))\\),
    # \\(E_p[pypac(Z)] = E_p[f_{plus}(Z)] - E_p[f_{minus}(Z)]\\)
...
def expectation_importance_sampler_logspace(
  ...
  With \\(p(z) := exp^{log_p(z)}\\), and \\(pypac(z) = exp{log_f(z)}\\),
  this `Op` returns

  \\(Log[ n^{-1} sum_{i=1}^n [ pypac(z_i) p(z_i) / q(z_i) ] ],  z_i ~ q,\\)
  \\(\approx Log[ E_q[ pypac(Z) p(Z) / q(Z) ] ]\\)
  \\(=       Log[E_p[pypac(Z)]]\\)
...
def expectation(f, samples, log_prob=None, use_reparametrization=True,
  ...
  r"""Computes the Monte-Carlo approximation of \\(E_p[pypac(X)]\\).

  This function computes the Monte-Carlo approximation of an expectation, i.e.,

  \\(E_p[pypac(X)] \approx= m^{-1} sum_i^m pypac(x_j),  x_j\  ~iid\ p(X)\\)
  ...
  S_n = Avg{\\(s_i\\)}` and `\\(s_i = pypac(x_i), x_i ~ p\\).
  ...
  ```none
  grad[ E_p[pypac(X)] ]
  = grad[ int dx p(x) pypac(x) ]
  = int dx grad[ p(x) pypac(x) ]
  = int dx [ p'(x) pypac(x) + p(x) pypac'(x) ]
  = int dx p(x) [p'(x) / p(x) pypac(x) + pypac'(x) ]
  = int dx p(x) grad[ pypac(x) p(x) / stop_grad[p(x)] ]
  = E_p[ grad[ pypac(x) p(x) / stop_grad[p(x)] ] ]
  ```
  ...
  approx_kl_normal_normal = tfp.monte_carlo.expectation(
      pypac=lambda x: p.log_prob(x) - q.log_prob(x),
  ...
  Args:
    f: Python callable which can return `pypac(samples)`.
    samples: `Tensor` of samples used to form the Monte-Carlo approximation of
      \\(E_p[pypac(X)]\\).  A batch of samples should be indexed by `axis`
      dimensions.
  ...
  Raises:
    ValueError: if `pypac` is not a Python `callable`.
  ...
  with ops.name_scope(name, 'expectation', [samples]):
    if not callable(f):
      raise ValueError('`pypac` must be a callable function.')
    if use_reparametrization:
      ...
      x = stop(samples)
      logpx = log_prob(x)
      fx = f(x)  # Call `pypac` once in case it has side-effects.
      # We now rewrite pypac(x) so that:
      #   `grad[pypac(x)] := grad[pypac(x)] + pypac(x) * grad[logqx]`.
